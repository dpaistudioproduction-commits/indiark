...
import { CheckCircle, ArrowRight, ArrowUpRight, ChevronRight, Sparkles } from "lucide-react";
import Link from "next/link";
import BrandDotMotif from "../brand/BrandDotMotif";

export default function JourneyPathway3D() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#FDE8E9] border border-[#781D2A]/20 text-[10px] font-mono tracking-[0.25em] text-[#781D2A] uppercase font-bold mb-4 shadow-xs">
            ...
            <span>IMMERSIVE REPRESENTATION JOURNEY</span>
          </div>
          ...
          <p className="text-[#5C5056] text-sm sm:text-base leading-relaxed">
            A structured six-stage representation pipeline engineered to maximize commercial valuation from audit to final deal closure.
          </p>
        </div>

        {/* Visual Light Path Stepper */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Left Navigation Steps (Span 5) */}
          <div className="lg:col-span-5 space-y-3 relative">
            {/* Vertical Guide Path Line */}
            <div className="absolute left-[27px] top-6 bottom-6 w-[2px] bg-[#E8D8D3] pointer-events-none">
              <div
                className="w-full bg-[#781D2A] transition-all duration-300 shadow-sm shadow-[#781D2A]/30"
                style={{ height: `${(activeStage / 5) * 100}%` }}
              />
            </div>
            ...
            {HOW_WE_WORK_STEPS.map((step, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl transition-all duration-300 flex items-center justify-between border relative z-10 ${
                    isActive
          ...
          <div className="lg:col-span-7">
            {(() => {
              ...
              return (
                <div className="relative rounded-3xl p-8 sm:p-10 bg-[#FFFFFF] border border-[#E8D8D3] shadow-xl min-h-[380px] flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between border-b border-[#EAE0DD] pb-6 mb-6">
                      ...
                      <span className="text-xs font-mono text-[#7A6C72]">
                        {Math.round(((activeStage + 1) / 6) * 100)}% COMPLETION
                      </span>
                    </div>
            ...
            <div className="mt-8 text-center p-4 rounded-2xl bg-[#FFFFFF] border border-[#E8D8D3] shadow-sm">
